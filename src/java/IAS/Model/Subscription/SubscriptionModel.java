...
public class SubscriptionModel extends JDSModel {
    ...
    private static final Logger logger = JDSLogger.getJDSLogger(SubscriptionModel.class.getName());
    ...
    public List<IAS.Bean.Subscription.SubscriptionDetail> getSubscriptionDetailsForSubscription(int subscriptionID) throws SQLException {
        ...
        try (Connection _conn = this.getConnection()) {
            // Execute the SQL statement and return the results in a List of
            // Person objects generated by the BeanListHandler.
            sub_details = run.query(_conn, Queries.getQuery("get_subscription_details_for_subscription"),
                    h, subscriptionID);
    ...
    public List<IAS.Bean.Subscription.SubscriptionFormBean> getSubscriptionsForInward(String inwardNumber) {
        
        ResultSetHandler<List<IAS.Bean.Subscription.SubscriptionFormBean>> h = new BeanListHandler<>(IAS.Bean.Subscription.SubscriptionFormBean.class);
        
        // No DataSource so we must handle Connections manually
        QueryRunner run = new QueryRunner();

        List<IAS.Bean.Subscription.SubscriptionFormBean> subscriptions = null;
        
        try (Connection _conn = this.getConnection()) {
            // Execute the SQL statement and return the results in a List of
            // Person objects generated by the BeanListHandler.
            subscriptions = run.query(_conn, Queries.getQuery("get_subscription_for_inward"),
                    h, inwardNumber);
